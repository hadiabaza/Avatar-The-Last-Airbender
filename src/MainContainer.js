...
function removeFavorite(character) {
    ...
    function favNations(nation) {
        const filteredFav = myCharacters.filter(character => character.affiliation === nation)
        setFavoriteFilter(filteredFav)
    }

    function favElements(element) {
        const filteredFav = myCharacters.filter(character => character.element === element)
        setFavoriteFilter(filteredFav)
    }

    function favShows(show) {
        const filteredFav = myCharacters.filter(character => character.show === show)
        setFavoriteFilter(filteredFav)
    }
            ...
            return (
                <div id="main-container">
                    ...
                    <span className="filter-btns">
                        ...
                        <button onClick={() => favNations("Water Tribe")} >Water Tribe</button>
                        <button onClick={() => favNations("Fire Nation")} >Fire Nation</button>
                        <button onClick={() => favNations("Earth Kingdom")} >Earth Kingdom</button>
                        <button onClick={() => favNations("Republic City")} >Republic City</button>
                    </span>
                ...
                return (
                    <div id="main-container">
                        ...
                        <br></br>
                        <span className="filter-btns">
                            <button onClick={() => favShows("Avatar, The Last Airbender")} >Avatar, The Last Airbender</button>
                            <button onClick={() => favShows("The Legend of Korra")} >The Legend of Korra</button>
                        </span>
                        <FavoritesContainer 
                        myCharacters={favoriteFilter.length === 0 ? myCharacters : favoriteFilter}
                        handleClick={removeFavorite}
